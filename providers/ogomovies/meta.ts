...
async function getDownloadLinks(
  ...
  try {
    ...
    const serverLink = $('li[data-server="3"][data-putload]').attr(
      "data-putload",
    );
    ...
    response = await axios.get(serverLink, {
    ...
    response = await axios.get(linkGateUrl, {
      headers: { ...headers, Referer: serverLink },
    });
    $ = cheerio.load(response.data);
    ...
    response = await axios.get(finalDownloadPageUrl, {
      headers: { ...headers, Referer: linkGateUrl },
    });
    $ = cheerio.load(response.data);
    ...
    response = await axios.get(cdnLinkUrl, {
      headers: { ...headers, Referer: finalDownloadPageUrl },
    });
    $ = cheerio.load(response.data);
...
export const getMeta = async function ({
  ...
  try {
    ...
    const qualityText =
      detailEl.find(".mvici-right .quality a").text().trim() || "Unknown";
    ...
    if (watchLinkUrl) {
      const deepDownloadLinks = await getDownloadLinks(
        watchLinkUrl,
        result.title,
        providerContext,
      );
